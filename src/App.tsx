...
import { Moon, Clock, Trash2, X, Sparkles, AlertCircle, Play, FolderOpen } from 'lucide-react';
...
import { DrivingSkin } from './components/DrivingSkin';
...
export default function App() {
  ...
  const [skin, setSkin] = useState<SkinType>(() => {
    const saved = safeGetItem('spinamp_skin_type') as SkinType;
    return saved === 'driving' ? 'driving' : 'bento';
  });
  const [skinColor, setSkinColor] = useState<string>(() => safeGetItem('spinamp_skin_color') || 'vespa-gold');
  ...
  const [customSkin, setCustomSkin] = useState(() => {
    try {
      const saved = safeGetItem('spinamp_custom_skin');
      if (saved) {
      ...
      console.warn('Error reading custom skin from storage:', e);
    }
    return {
      name: 'My Custom Skin',
      bg: '#12131c',
  ...
  useEffect(() => {
    safeSetItem('spinamp_screen_appearance', screenAppearance);
  ...
  useEffect(() => {
    safeSetItem('spinamp_screen_font', screenFont);
  }, [screenFont]);

  const [tracks, setTracks] = useState<Track[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = safeGetItem('spinamp_playlist_meta');
      if (saved) {
        try {
          ...
          console.warn("Error parsing spinamp_playlist_meta", e);
        }
      }
    }
  ...
  const [currentTrack, setCurrentTrack] = useState<Track | null>(() => {
    if (typeof window !== 'undefined') {
      const savedId = safeGetItem('spinamp_last_track_id');
      if (savedId) {
        const savedMeta = safeGetItem('spinamp_playlist_meta');
  ...
  const [visualizerMode, setVisualizerMode] = useState<VisualizerMode>(() => {
    return (safeGetItem('spinamp_vis_mode') as VisualizerMode) || 'spectrum';
  });
  const [visTheme, setVisTheme] = useState<string>(() => safeGetItem('spinamp_vis_theme') || 'neon');
  const [visSensitivity, setVisSensitivity] = useState<number>(() => {
  ...
  const [timeDisplayMode, setTimeDisplayMode] = useState<'elapsed' | 'remaining'>(() => {
    return (safeGetItem('spinamp_time_display_mode') as 'elapsed' | 'remaining') || 'elapsed';
  });
  ...
  // User action notice state (e.g. autoplay blocked or manual file re-selection prompt)
  ...
  useEffect(() => {
    safeSetItem('spinamp_vis_mode', visualizerMode);
  }, [visualizerMode]);

  useEffect(() => {
    safeSetItem('spinamp_time_display_mode', timeDisplayMode);
  }, [timeDisplayMode]);
  ...
  useEffect(() => {
    ...
    if (currentTrack) {
      safeSetItem('spinamp_last_track_id', currentTrack.id);
    }
  }, [currentTrack]);
  ...
  const handleSkinColorChange = useCallback((newColor: string) => {
    ...
    safeSetItem('spinamp_skin_color', newColor);
    ...
    if (correspondingVisTheme) {
      ...
      safeSetItem('spinamp_vis_theme', correspondingVisTheme);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const handleSkinEvent = () => {
      setSkin((prev) => (prev === 'bento' ? 'driving' : 'bento'));
    };
    ...
    const cachedStats = safeGetItem('spinamp_retro_stats');
    if (cachedStats) {
      try {
    ...
    const savedTrackId = safeGetItem('spinamp_last_track_id');
    const savedMeta = safeGetItem('spinamp_playlist_meta');
    let allTracks = defaultTracks;
    if (savedMeta) {
      try {
  ...
  useEffect(() => {
    ...
    reopenFilesViaAndroidBridge(uris).then(async ({ reopened, failedUris }) => {
      ...
      const activeTrack = currentTrackRef.current;
      const failedUriSet = new Set(failedUris);
      const activeTrackFailedReopen = Boolean(
        activeTrack && activeTrack.androidUri && failedUriSet.has(activeTrack.androidUri)
      );

      if (activeTrackFailedReopen && activeTrack) {
        // Requirement 3: If reopen for currently selected track failed, do NOT call handleReimportTrack automatically.
        // Show a visible notice/button to manually re-select the file instead.
        pendingPlayAfterReopenRef.current = false;
        setLoadingFilesMessage(null);
        setUserActionNotice({
          message: `Datei für "${activeTrack.title || 'Track'}" konnte nicht wiederhergestellt werden.`,
          buttonText: 'Datei neu auswählen',
          action: () => handleReimportTrack(activeTrack.id),
          type: 'reselectFile',
        });
      } else if (pendingPlayAfterReopenRef.current) {
        pendingPlayAfterReopenRef.current = false;
        // Requirement 2: Keep loadingFilesMessage visible until playback actually starts or error is shown.
        setLoadingFilesMessage('Verbindung hergestellt — starte Wiedergabe...');

        try {
          await new Promise((resolve) => setTimeout(resolve, 50));
          await handlePlayRef.current();
          // Success! Playback started!
          setLoadingFilesMessage(null);
          setUserActionNotice(null);
        } catch (playErr) {
          console.warn('Autoplay blocked after reopen:', playErr);
          // Requirement 1: Show a visible notice/button ("Tippen zum Abspielen") that triggers play() on tap.
          setLoadingFilesMessage(null);
          setUserActionNotice({
            message: 'Autoplay blockiert — bitte tippen zum Abspielen.',
            buttonText: 'Tippen zum Abspielen',
            action: async () => {
              try {
                await handlePlayRef.current();
                setUserActionNotice(null);
              } catch (e) {
                console.warn('User-initiated play failed:', e);
              }
            },
            type: 'tapToPlay',
          });
  ...
  const saveStatsToStorage = useCallback((updatedTracks: Track[]) => {
    ...
    safeSetItem('spinamp_retro_stats', JSON.stringify(stats));
  }, []);
  ...
  const handleAddFiles = useCallback(async (files: FileList | File[]) => {
    ...
    const existingFileSignatures = new Set(
      currentTracks
        .filter((t) => t.file)
        .map((t) => `${t.file?.name || ''}:${t.file?.size || 0}`)
    );

    const newFiles = audioFiles.filter((file) => {
      const signature = `${file?.name || ''}:${file?.size || 0}`;
    ...
    for (let i = 0; i < newFiles.length; i++) {
      ...
      const fallbackTitle = file.name ? file.name.replace(/\.[^/.]+$/, "") : "Untitled Track";
      ...
      try {
        ...
        if (matchingIdx !== -1) {
          ...
          const t: Track = {
            ...
            artist: metadata.artist || "Local Singer",
            album: metadata.album || "Local Folder Upload",
            duration: metadata.duration || 180,
            file: file,
        ...
        console.error("Error creating track for file: ", err);
      }
    }
  ...
  const handleAutoTriggerLlamaOnPowerOn = useCallback(() => {
    setIsPowerOn(true);
    const hasLaunched = safeGetItem('spinamp_has_launched_before') === 'true';
    if (!hasLaunched) {
      spinampAudio.triggerLlamaAss();
      safeSetItem('spinamp_has_launched_before', 'true');
    }
  }, []);
  ...
  const handleResetToDefaults = useCallback(() => {
    ...
    safeRemoveItem('spinamp_skin_color');
    safeRemoveItem('spinamp_vis_theme');
    safeRemoveItem('spinamp_vis_sensitivity');
    safeRemoveItem('spinamp_screen_appearance');
    safeRemoveItem('spinamp_screen_font');
  ...
  const handleToggleVisualizerMode = useCallback(() => {
    const modes: VisualizerMode[] = [
      ...
      'guitar-hero', 'artwork', 'vegas-strip', 'skytree', 'lava-lamp', 'synthwave-grid', 'plasma-globe', 'keygen', 'demoscene', 'random', 'off'
    ];
  ...
  return (
    ...
    >
        ...
        <div id="active-applet-drawer" className={`flex-1 flex flex-col min-h-0 min-w-0 screen-${screenAppearance} font-preset-${screenFont}`}>
          ...
          {userActionNotice && (
            <div
              ...
              className="bg-amber-500/20 border-b border-amber-500/50 text-amber-200 px-3 py-2 text-xs font-mono flex items-center justify-between gap-3 z-50 shrink-0 shadow-md animate-fade-in"
            >
              <div className="flex items-center gap-2 truncate min-w-0">
                <AlertCircle className="w-4 h-4 text-amber-400 shrink-0 animate-pulse" />
                <span className="truncate">{userActionNotice.message}</span>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => {
                    const act = userActionNotice.action;
                    setUserActionNotice(null);
                    act();
                  }}
                  className="px-2.5 py-1 bg-amber-500 hover:bg-amber-400 text-black font-bold text-xs uppercase rounded transition cursor-pointer shadow flex items-center gap-1.5 shrink-0"
                >
                  {userActionNotice.type === 'tapToPlay' && <Play className="w-3 h-3 fill-black text-black" />}
                  {userActionNotice.type === 'reselectFile' && <FolderOpen className="w-3 h-3 text-black" />}
                  {userActionNotice.buttonText}
                </button>
                <button
                  type="button"
          ...
          {skin === 'driving' ? (
            ...
            <BentoSkin
              ...
              onSetLoadingMessage={setLoadingFilesMessage}
      ...
      <CustomSkinModal
        ...
        onApply={() => {
          safeSetItem('spinamp_custom_skin', JSON.stringify(customSkin));
